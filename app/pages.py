...
def register():
    ...
    if new_password != "":
        ...
        if new_password == new_password_again:
            new_gender = st.selectbox("請輸入性別:", ("男", "女"))
            ...
            if st.button("提交"):
                with open("register_info.pickle", "wb") as f:
                    pickle.dump(
                        (new_username, new_gender, birth_day, moeny),
                        f,
...
def cart(buy_number_list):
    ...
    buy_product_list = [
        [p] * buy_number_list[i] for i, p in enumerate(product_dict.keys())
    ]
    ...
    try:
        ...
        st.write(
            f"### {discount(sum([product_dict[p] for p in final_buy_product_list]))[1]}"
        )
